...
public class DivideController {
	...
	public String main(HttpServletRequest request, HttpServletResponse response, DivideVO vo, MemberVO mvo) throws Exception {
		...
		for(int i = 0; i < list.size(); i ++) {
			...
			String fileList = list.get(i).getFilepath().toString();			
			
			if(fileList.contains(".txt")) {
				
			 list.get(i).getFilepath().replace(list.get(i).getFilepath(), "noimage.jpg");
				
				
				System.out.println("변경후 >> " + list.get(i).getFilepath().toString());
			
			}
			
			System.out.println("fileList >> " + fileList);	
			System.out.println(" test >> " + fileList.contains(".txt"));
		}
